...
bool DES::setKey(const unsigned char* keyArray)
{
	 ...
	 */
	 ...
	 cout << "We are withink DES object calling setkey function\n"; //DELETE THIS
	/* The key error code */
	int keyErrorCode = -1;
...
unsigned char* DES::encrypt(const unsigned char* plaintext)
{
	...
	for(int i = 0; i < 4; i++){
		...
		first_block[i] = plaintext[i];

	}
	block[0] = ctol(first_block);

	//Req 3
	unsigned char second_block[4];
	for(int i = 4; i < 8; i++){

		second_block[i-4] = plaintext[i];

	}
	block[1] = ctol(second_block);

	//Req 4
	DES_encrypt1(block, &this->key , ENC);  //encryption stored in block
																						//define enc and dec as 0 and 1 and put them in last area
	//Req 5
	unsigned char first_ciphertxt[4];
	ltoc(block[0], first_ciphertxt);

	//Req 6
	unsigned char second_ciphertxt[4];
	ltoc(block[1], second_ciphertxt);

	//combine first_ciphertxt with second_ciphertxt in one array
	unsigned char* bytes = new unsigned char[8];
	for(int i = 0; i < 4; i++){

		bytes[i] = first_ciphertxt[i];

	}
	for(int i = 4; i < 8; i++){

		bytes[i] = second_ciphertxt[i-4];
...
unsigned char* DES::decrypt(const unsigned char* ciphertext)
{
	...
	for(int i = 4; i < 8; i++){
		...
		second_block[i-4] = ciphertext[i];

	}
	block[1] = ctol(second_block);

	//Req 4
	DES_encrypt1(block, &this->key , DEC);  //encryption stored in block

	//Req 5
	unsigned char first_plaintxt[4];
	ltoc(block[0], first_plaintxt);

	//Req 6
	unsigned char second_plaintxt[4];
	ltoc(block[1], second_plaintxt);

	//combine first_ciphertxt with second_ciphertxt in one array
	unsigned char* bytes = new unsigned char[8];
	for(int i = 0; i < 4; i++){

		bytes[i] = first_plaintxt[i];

	}
	for(int i = 4; i < 8; i++){

		bytes[i] = second_plaintxt[i-4];
